...
>
> 작성일: 2026-06-18 · 상태: Draft (feasibility 검증 반영)
>
> 선행 검증: [feasibility.md](../feasibility.md) — 경로 API·충전소 데이터 실재 확인 완료
...
| 지도 표시 | 네이버/카카오 지도 SDK (국내 친화) 또는 Mapbox | 약관·쿼터 비교 후 결정 |
...
| 경로 서비스 | Node.js/TypeScript (외부 경로 API 오케스트레이션) | 도로 엔진 자체 운영 X, 정차 최적화만 자체 구현 |
| 도로 경로 | 네이버 Directions / 카카오모빌리티 / TMAP 중 택1 | 경로·통행료·경로 매트릭스 제공 (ADR-0003) |
| 충전소 데이터 | 한국환경공단 충전기 API (`B552584/EvCharger`) | 사양·실시간 상태(휴게소 포함), Supabase로 동기화 |
| 배포 | Vercel(웹) / EAS(모바일) / Fly.io·Render(라우팅 서비스) | |

> 도로 라우팅 엔진(OSRM/Valhalla)은 직접 운영하지 않는다. feasibility 검증 결과 국내 경로 API로
> 충당 가능하며, 우리의 차별점은 충전 정차 최적화 로직이다. (feasibility.md, ADR-0003 참고)
...
1. **기본 경로 산출**: 외부 경로 API(네이버/카카오/TMAP)로 도로 경로 + 구간별 거리/시간 획득.
2. **에너지 모델**: 구간별 소비 에너지 추정 (거리·표고·속도·기본 전비, Phase 2에서 기상/온도 보정).
3. **도달 가능성 검사**: 무충전 도달 가능하면 정차 없음.
4. **후보 충전소 수집**: 경로 회랑(corridor) 내 충전소를 PostGIS 반경 질의로 수집(환경부
   API 동기화 캐시), 차량 커넥터 호환 필터. 후보 간 거리·시간은 경로 매트릭스 API로 일괄 산출.
...
- 알고리즘은 `packages/core`에 **순수 함수**로 두고, 외부 경로 API 호출은
  주입(adapter)으로 분리 → 단위 테스트 가능, provider 교체 가능.
...
| 데이터 동기화 | Cron Edge Function이 환경부 충전기 API 증분 동기화(쿼터 보호용 캐시) |
